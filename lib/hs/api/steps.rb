require 'hs/api/response'
require 'hs/api/helpers'
require 'rest-client'
require 'json-schema'
...
When(/^I send a (GET|POST|PATCH|PUT|DELETE) request to "(.*?)"$/) do |method, url|
  ...
  @response = HsCucumberApi::Response.create response
  @headers = nil
  @body = nil

  $cache[%/#{request_url}/] = @response if 'GET' == %/#{method}/
  Allure.add_attachment(name: "response", source: @response, type: Allure::ContentType::JSON, test_case: false )
end
